            ...
            for k, v in expectedDict.items():  # 比较字典
                ...
                if k not in aimDict:  # 实际值缺少这个key直接结束匹配
                    ...
                    if isinstance(v, dict):  # v为字典时进入此逻辑
                        ...
                        expected_sorted = sorted(v, key=lambda x: str(x) if isinstance(x, dict) or (isinstance(x, list) and isinstance(x[0], dict)) else x)  # 对列表进行排序，忽略顺序
                        actual_sorted = sorted(aimDict[k], key=lambda x: str(x) if isinstance(x, dict) or (isinstance(x, list) and isinstance(x[0], dict)) else x)  # 对列表进行排序，忽略顺序
